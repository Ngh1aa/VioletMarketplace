(() => {
  ...
  function setTrio(ids){ localStorage.setItem(TRIO_KEY, JSON.stringify([...new Set(ids)].slice(0,3))); document.dispatchEvent(new CustomEvent('violet:trio-change')); }
  ...
  function v4Card(product, extra=''){
    const meta=metaFor(product); const house=houseFor(product);
    return `<article class="v4-object-card">
      <a class="v4-object-link" href="product.html?id=${encodeURIComponent(product.id)}">
        <div class="v4-object-media"><img src="${product.image}" alt="${product.name} by ${product.brand}" loading="lazy"></div>
        <div class="v4-object-index"><span>${house ? house.origin : 'Violet edit'}</span><span>${meta.sample ? 'Trial eligible' : product.family || ''}</span></div>
  ...
  function upgradeChrome(){
    ...
    const top=document.querySelector('.topbar-inner > span'); if(top) top.textContent='Violet Editions · fragrance, houses and slower discovery';
    const topLinks=document.querySelector('.top-links'); if(topLinks) topLinks.innerHTML='<a href="discovery.html">Discovery ritual</a><a href="finder.html">Scent concierge</a><a href="seller.html">For fragrance houses</a>';
    ...
    if(footer) footer.innerHTML=`<div><a class="brand footer-brand" href="index.html">Violet<span>.</span><small>Parfumerie</small></a><p>A fictional multi-house fragrance marketplace prototype built around objects, maisons and slower trial.</p></div><div><h4>Discover</h4><div class="footer-links"><a href="search.html">Fragrances</a><a href="discovery.html">Discovery</a><a href="finder.html">Scent Portrait</a></div></div><div><h4>Houses</h4><div class="footer-links"><a href="houses.html">Curated houses</a><a href="seller.html">Fragrance House Center</a><a href="search.html?sample=1">Trial-eligible scents</a></div></div><div><h4>Reality</h4><div class="footer-links"><span>Prototype catalogue</span><span>Simulated checkout</span><span>No fake AI claims</span></div></div>`;
  }

  function renderHome(){
    const feature=document.querySelector('[data-v4-feature]'); if(!feature) return;
    const product=byId('violette-03') || DATA.products[0]; const meta=metaFor(product); const house=houseFor(product);
    feature.innerHTML=`<div class="v4-feature-copy"><div class="v4-section-label"><span>V.01 / FEATURED OBJECT</span><span>${house?.origin || 'Violet edit'}</span></div><div><span class="v4-kicker">${product.brand}</span><h1>${product.name}</h1><p class="v4-feature-sensory">${meta.character}</p></div><div class="v4-feature-commerce"><span>${product.concentration} · ${product.size}</span><strong>${money(product.price)}</strong><div><a class="btn btn-primary" href="product.html?id=${product.id}">Explore the object</a><a class="v4-text-link" href="discovery.html?focus=${product.id}">Try the direction →</a></div></div></div><a class="v4-feature-media" href="product.html?id=${product.id}"><span>OBJECT STUDY / ${product.family}</span><img src="${product.image}" alt="${product.name} by ${product.brand}"><i>Violet edit · 01</i></a>`;

    const houseIndex=document.querySelector('[data-v4-house-index]'); if(houseIndex) houseIndex.innerHTML=`<div class="v4-house-index">${HOUSES.slice(0,5).map((house,index)=>`<a href="house.html?id=${house.id}"><span>${String(index+1).padStart(2,'0')}</span><strong>${house.name}</strong><em>${house.origin}</em><small>${house.territory}</small><i>↗</i></a>`).join('')}</div>`;

    const shelf=document.querySelector('[data-v4-shelf]'); if(shelf){ const ids=['peau-de-lune','velours-ambre','neroli-rain','santal-veil']; const objects=ids.map(byId).filter(Boolean); shelf.innerHTML=`${objects.slice(0,2).map(p=>v4Card(p)).join('')}<aside class="v4-shelf-interrupt"><span>VIOLET EDIT / SKIN & SHADOW</span><strong>Quiet does not mean anonymous.</strong><p>Move between an intimate skin musk and darker evening warmth without turning the shelf into a popularity chart.</p><a href="search.html?mood=intimate">Open an intimate edit →</a></aside>${objects.slice(2).map(p=>v4Card(p)).join('')}`; }
  }
  ...
  function renderLibrary(){
    ...
    panel.innerHTML=`<div class="v4-filter-title"><span>REFINE</span><a href="search.html">Clear all</a></div>
    ...
    const title=document.querySelector('[data-result-title]'); if(title){ if(q) title.textContent=`Results for “${current.get('q')}”`; else if(house) title.textContent=houseById(house)?.name || 'House edit'; else if(category) title.textContent=familyLabels[category]||'Fragrance'; else if(mood) title.textContent=moodLabels[mood]||'Mood edit'; else if(sample) title.textContent='Trial-eligible fragrance'; else title.textContent='Fragrance Library'; }
    const chips=document.querySelector('[data-active-chips]'); if(chips){ const active=[]; if(category) active.push(['category',familyLabels[category]||category]); if(mood) active.push(['mood',moodLabels[mood]||mood]); if(presence) active.push(['presence',presenceLabels[presence]||presence]); if(house) active.push(['house',houseById(house)?.name||house]); if(concentration) active.push(['concentration',concentration==='edp'?'Eau de Parfum':'Extrait']); if(sample) active.push(['sample','Trial eligible']); if(q) active.push(['q',`“${current.get('q')}”`]); chips.innerHTML=active.map(([key,label])=>`<span class="chip">${label}<a href="${removeParamUrl(key)}" aria-label="Remove ${label}">×</a></span>`).join(''); }

    const sort=document.querySelector('[data-sort]');
    const draw=()=>{ let list=[...items]; if(sort?.value==='price-asc') list.sort((a,b)=>a.price-b.price); if(sort?.value==='price-desc') list.sort((a,b)=>b.price-a.price); if(sort?.value==='rating') list.sort((a,b)=>b.rating-a.rating); document.querySelectorAll('[data-result-count]').forEach(el=>el.textContent=`${list.length} object${list.length===1?'':'s'}`); if(!list.length){ grid.innerHTML=`<div class="v4-empty"><span>NO OBJECT IN THIS EDIT</span><h2>Widen the scent portrait.</h2><p>Remove one refinement or let Violet narrow from feeling instead.</p><div><a class="btn btn-secondary" href="search.html">Clear filters</a><a class="btn btn-primary" href="finder.html">Create Scent Portrait</a></div></div>`; return; } const chunks=[]; list.forEach((product,index)=>{ chunks.push(v4Card(product)); if(index===5 && list.length>6) chunks.push(`<aside class="v4-plp-interrupt"><span>HOUSE INTERRUPTION / 03</span><strong>Browse a maison, not only a family.</strong><p>When notes are not enough, enter the collection through the house’s scent territory.</p><a href="houses.html">Open curated houses →</a></aside>`); }); grid.innerHTML=chunks.join(''); };
    sort?.addEventListener('change',draw); draw();
  }

  function relatedProducts(product){ const meta=metaFor(product); return DATA.products.filter(item=>item.id!==product.id && metaFor(item).sample).map(item=>{ const im=metaFor(item); let score=0; const reasons=[]; if(item.category===product.category){score+=4;reasons.push(`same ${product.family||'family'} direction`);} const shared=im.mood.find(value=>meta.mood.includes(value)); if(shared){score+=3;reasons.push(`shared ${moodLabels[shared]?.toLowerCase()||shared} mood`);} if(im.presence===meta.presence){score+=2;reasons.push(`similar ${presenceLabels[meta.presence]?.toLowerCase()||'presence'}`);} return {item,score,reason:reasons.slice(0,2).join(' · ')||'a contrasting route in the Violet edit'}; }).sort((a,b)=>b.score-a.score).slice(0,3); }

  function renderProduct(){
    const host=document.querySelector('[data-product-detail]'); if(!host) return; const product=byId(qs.get('id'))||DATA.products[0]; if(!product) return; const meta=metaFor(product); const house=houseFor(product); const notes=product.notes||{}; const related=relatedProducts(product); document.title=`${product.name} · ${product.brand} · Violet Parfumerie`;
    host.className='v4-pdp';
    host.innerHTML=`<div class="v4-pdp-media"><div class="v4-pdp-media-index"><span>V.11 / OBJECT STUDY</span><span>${product.family||'Fine fragrance'}</span></div><div class="v4-pdp-image"><img src="${product.image}" alt="${product.name} by ${product.brand}"></div><div class="v4-pdp-media-foot"><span>${product.brand}</span><span>${product.concentration||''} · ${product.size||''}</span></div></div>
      <aside class="v4-buying-desk"><div class="v4-buying-index"><span>VIOLET OBJECT / ${String(DATA.products.findIndex(item=>item.id===product.id)+1).padStart(2,'0')}</span><span>${meta.sample?'TRIAL ELIGIBLE':'VIOLET EDIT'}</span></div><a class="v4-pdp-house" href="${house?`house.html?id=${house.id}`:'houses.html'}">${product.brand}</a><h1>${product.name}</h1><p class="v4-pdp-character">${meta.character}</p><div class="v4-pdp-format">${product.concentration||''} · ${product.size||''} · ${product.family||''}</div><div class="v4-pdp-price"><strong>${money(product.price)}</strong><span>${product.stock>0?`${product.stock} in prototype inventory`:'Unavailable'}</span></div><div class="v4-size-row"><span>Size</span><button type="button" aria-pressed="true">${product.size||'One size'}</button></div><div class="v4-pdp-actions">${meta.sample?`<button class="btn btn-secondary" type="button" data-v4-add-sample="${product.id}">Add direction to trio</button>`:`<a class="btn btn-secondary" href="discovery.html">Explore Discovery</a>`}<button class="btn btn-primary" type="button" data-v4-add-bottle="${product.id}" ${product.stock<=0?'disabled':''}>Add full bottle</button></div><p class="v4-reality-note">${meta.sample?'Representative trial selections are stored locally in this prototype; no sample fulfillment or payment is processed.':'Prototype catalogue item; commerce behavior is browser-local.'}</p><div class="v4-pdp-quick"><div><span>Presence</span><strong>${presenceLabels[meta.presence]||meta.presence}</strong></div><div><span>Wear</span><strong>${meta.wear}</strong></div></div></aside>
      <section class="v4-pdp-story"><div class="v4-pdp-story-lead"><span class="v4-kicker">01 / THE SCENT</span><h2>${product.description}</h2></div><div class="v4-note-table"><div><span>Top</span><strong>${notes.top||'—'}</strong></div><div><span>Heart</span><strong>${notes.heart||'—'}</strong></div><div><span>Base</span><strong>${notes.base||'—'}</strong></div></div><div class="v4-house-proof"><span class="v4-kicker">02 / THE HOUSE</span><h3>${house?.name||product.brand}</h3><p>${house?.ethos||'Part of the Violet curated prototype edit.'}</p><p class="v4-house-territory">${house?.territory||product.family||''}</p><a href="${house?`house.html?id=${house.id}`:'houses.html'}">Open the maison →</a></div></section>
      <section class="v4-related"><div class="v4-section-label"><span>03 / NEARBY DIRECTIONS</span><span>Reasoned by shared family, mood or presence</span></div><div class="v4-related-grid">${related.map(row=>v4Card(row.item,row.reason)).join('')}</div></section>`;
  ...
  function renderTrio(){
    const panel=document.querySelector('[data-v4-trio-panel]'); if(!panel) return; const trio=getTrio(); const products=trio.map(byId).filter(Boolean); panel.innerHTML=`<div class="v4-trio-head"><span>YOUR TRIO</span><strong>${products.length}/3</strong></div>${products.length?`<div class="v4-trio-items">${products.map((product,index)=>`<div class="v4-trio-item"><span>${String(index+1).padStart(2,'0')}</span><div><strong>${product.name}</strong><small>${product.brand} · ${product.family}</small></div><button type="button" data-v4-remove-sample="${product.id}" aria-label="Remove ${product.name}">Remove</button></div>`).join('')}</div>`:`<div class="v4-trio-empty"><strong>No direction selected yet.</strong><p>Choose up to three representative fragrances from the list below.</p></div>`}<div class="v4-trio-state"><p>${products.length===3?'Trio complete — wear each direction separately, then return to the bottles that remain convincing.':`${3-products.length} place${3-products.length===1?'':'s'} remaining.`}</p>${products.length?`<div class="v4-trio-links">${products.map(product=>`<a href="product.html?id=${product.id}">${product.name} →</a>`).join('')}</div>`:''}</div>`; panel.querySelectorAll('[data-v4-remove-sample]').forEach(button=>button.addEventListener('click',()=>{removeFromTrio(button.dataset.v4RemoveSample);renderTrio();renderSampleList();}));
  }

  function renderSampleList(){
    const host=document.querySelector('[data-v4-sample-list]'); if(!host) return; const trio=getTrio(); const products=DATA.products.filter(product=>metaFor(product).sample); host.innerHTML=products.map((product,index)=>{ const selected=trio.includes(product.id); const full=trio.length>=3&&!selected; return `<article class="v4-sample-row"><span>${String(index+1).padStart(2,'0')}</span><div><strong>${product.name}</strong><small>${product.brand} · ${product.family}</small></div><p>${metaFor(product).character}</p><button type="button" data-v4-sample-choice="${product.id}" aria-pressed="${selected}" ${full?'disabled':''}>${selected?'Selected':full?'Trio full':'Add to trio'}</button></article>`; }).join(''); host.querySelectorAll('[data-v4-sample-choice]').forEach(button=>button.addEventListener('click',()=>{ const id=button.dataset.v4SampleChoice; if(getTrio().includes(id)) removeFromTrio(id); else { const result=addToTrio(id); status(result.message); } renderTrio(); renderSampleList(); }));
  }

  function renderDiscovery(){
    if(!document.querySelector('.v4-discovery-page')) return; renderTrio(); renderSampleList(); const focus=qs.get('focus'); if(focus && byId(focus) && metaFor(byId(focus)).sample && !getTrio().includes(focus)){ const result=addToTrio(focus); if(result.ok){renderTrio();renderSampleList();} }
    const wardrobe=document.querySelector('[data-v4-wardrobe]'); const product=byId('violet-discovery-set'); if(wardrobe&&product) wardrobe.innerHTML=`<div><span class="v4-kicker">03 / PRE-COMPOSED</span><h2>Prefer one ready-made wardrobe?</h2><p>${product.description}</p><div class="v4-wardrobe-meta"><span>${product.size}</span><strong>${money(product.price)}</strong></div><div class="v4-wardrobe-actions"><button class="btn btn-primary" type="button" data-v4-add-wardrobe>Add Discovery Wardrobe to bag</button><a class="v4-text-link" href="product.html?id=${product.id}">View object →</a></div></div><div class="v4-wardrobe-media"><img src="${product.image}" alt="${product.name}"></div>`; wardrobe?.querySelector('[data-v4-add-wardrobe]')?.addEventListener('click',()=>addBottle(product.id));
    document.addEventListener('violet:trio-change',()=>{renderTrio();renderSampleList();},{once:true});
  }
